    ...
    <div class="create-container">
      ...
      <form id="song-form">
        ...
        <div class="select-wrapper">
          <select id="songGenre" name="genre">
  ...
  <script>
    ...
    function closeAlert() {
      ...
      if (alert.dataset.redirect) {
        setTimeout(() => { window.location.href = alert.dataset.redirect; }, 300);
      }
    }
    ...
    document.getElementById('song-form').addEventListener('submit', async function(e) {
      ...
      if (parts.length === 0) { showAlert('error', 'Error', 'Please add at least one song part with lyrics'); return; }

      try {
        const response = await fetch('save_song.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ title, subtitle, genre, parts })
        });
